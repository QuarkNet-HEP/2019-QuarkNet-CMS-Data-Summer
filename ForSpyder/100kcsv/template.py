# -*- coding: utf-8 -*-
"""
Created on Mon Jul 15 08:59:06 2019
...
import csv ## Library used to read csv files
import numpy as np ## library used to do operation involving matrices
import matplotlib.pyplot as plt # library used to plot out data
import math ## used to do mathematical operations and expressions.

nP=99999 #length


data_path = 'TYPE CSV NAME HERE'
with open(data_path, 'r') as f:
    ...
    data = np.array(data)
    data1 = data[:, 1:] ## data involving then numbers
    data2 = data[:, 0] ## data involving wether they are global or not
    data1=np.array(data1).astype(float) # sets data as floats that way we can use decimals to get more accurate readings.
    

##data1 =  np.delete(data1, 'invalid', axis=0)
##data2 =  np.delete(data2, 'invalid', axis=0) # in the case one of the data points end up invalid or there is an error just 
    ## find where it does that and replace invalid with that spot and uncomment lines 41 and 42
